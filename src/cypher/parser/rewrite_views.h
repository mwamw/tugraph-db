 ...
 */
class RewriteViews : public LcypherVisitor {
 ...
 public:
    ...
    std::string GetRewriteQuery() const { return rewrite_query; }
    std::string GetViewName() const { return view_name; }
    std::pair<std::string,std::string> GetConstraints() const {return constraints;}

    std::any visitOC_Cypher(LcypherParser::OC_CypherContext *ctx) override {
        LOG_DEBUG() <<"Cypher start";
    ...
    std::any visitOC_Statement(LcypherParser::OC_StatementContext *ctx) override {
        LOG_DEBUG() <<"Statement start";
        if(ctx->oC_View()==nullptr){
            throw lgraph::CypherException("Not Views");
        }
        return std::any_cast<std::string>(visit(ctx->oC_View()));
    }
