 ...
 public:
    ...
    std::any visitOC_RelationshipDetail(LcypherParser::OC_RelationshipDetailContext *ctx) override {
        ...
        if (ctx->oC_RangeLiteral() != nullptr) {
            std::string range_literal = std::any_cast<std::string>(visit(ctx->oC_RangeLiteral()));
        ...
        result.append("]");
        auto relp_detail_with_direction = std::make_tuple(relp->direction_, result);
        return relp_detail_with_direction;
    }
